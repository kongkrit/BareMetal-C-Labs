...
* **Rule 1.1: Parenthesize Macro Replacements.**
  ...
  * **Bad (and dangerous):** `#define BASE_ADDR (volatile uint8_t *) 0x8000U`
  * **Good:** `#define BASE_ADDR ((volatile uint8_t *) 0x8000U)`

* **Rule 1.2a: Mandatory `U` Suffix for Hardware Constants.**
  * **Context:** Hardware values (addresses, patterns, masks) must be Unsigned 16-bit integers to prevent sign-extension bugs.
  * **Case 1: Memory Addresses**
    * **Bad:** `#define START ((volatile uint8_t *)0x8000)` (Interpreted as negative `-32768`)
    * **Good:** `#define START ((volatile uint8_t *)0x8000U)`
  * **Case 2: Bit Patterns (Data)**
    * **Bad:** `*LEDS = 0xFF;` (Treated as decimal 255, not a raw pattern)
    * **Good:** `*LEDS = 0xFFU;`
  * **Case 3: Bit Masks & Shift Generators**
    * **Bad:** `(1 << 15)` (Undefined Behavior: shifting into sign bit)
    * **Good:** `(1U << 15)

* **Rule 1.2b: No `U` Suffix for Scalar Quantities.**
  * **Context:** "Counts" and "Amounts" (how *many* or how *far*) are just numbers, not bit patterns. They do not need a suffix.
  * **Example 1:** `pattern << 3;` (`3` is a distance)
  * **Example 2:** `for (int i=0; i < 10; ...)` (`10` is a count).
...
* **Rule 3.2: Define All Hardware Addresses via Macros.**
  * **No Magic Numbers:** Never use raw address literals (e.g., `0x8000U`) directly in your function logic. Give every hardware port a meaningful name.
  * **Format:** Define the address as a fully cast pointer in a single macro.
  * **Format Template:** `#define NAME ((volatile type *) addressU)`
  * *Example:*
    ```C
    // BAD (Magic Number):
    * ((volatile uint8_t *) 0x4000U) = 0xFFU;
